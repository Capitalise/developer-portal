...
| /marketplace/v2/auth/tenants/authorize | [/platform/v1/auth/token](/api/platform#tag/Auth/operation/post-auth-token) | Use API key and provide `Capitalise-tenant-id` in the header to get `tenant access token`, which is the token used for the JavaScript |
